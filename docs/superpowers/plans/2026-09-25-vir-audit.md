...
1. **Verdicts feed `vir review`; `vir audit` never moves, rejects or hides a note.**
   ...
   - The human stays the final say for `verify` and `merge`.
   - **Amended 2026-09-25 (Task 8):** `vir audit --apply-rejects` may act on **reject** verdicts only. It's an explicit separate invocation that makes no LLM call, and every move is reversible: `.rejected/` + `rejected_at` + `markRejected`, undone by `vir review --restore`. It ships only if calibration rule 5 passes. The reason: a reject is cheap to undo and its error rate is measured, while `verify` and `merge` mean editing text, which models do badly.
...
- [ ] **Step 4: Apply the frozen success rules.** The command ships only if **all** of these hold:
  ...
  5. *(Added 2026-09-25, before run 2's results existed.)* `--apply-rejects` (Task 8) ships only if the human agrees with **≥ 6 of the 7** sampled rejects in Step 3. If this rule fails but rules 1–3 pass, ship `vir audit` without `--apply-rejects`.
...
### Task 8: `vir audit --apply-rejects`

**Files:**
- Create: `src/audit/apply.ts`, `src/audit/apply.test.ts`
- Modify: `src/cli/review.ts` (`restoreRejected` also strips `rejected_by`), `src/cli/review.test.ts`, `src/cli.ts` (the `audit` command), `README.md`, `CHANGELOG.md`. Also the worktree copy of `CLAUDE.md`, which is git-ignored, so it's ported at release per the ledger ruling.

**Interfaces:**
- Consumes:
  - `StateDb.listDistilled()`, `StateDb.listAudits()` and `StateDb.markRejected(sessionId, now)`
  - `rejectNote(filePath, vaultRoot, now)` and `setFrontmatter` from `src/cli/review.ts`
  - `CATEGORY_DIR` from `src/pipeline/writer.ts`, `makeSlug` from `src/pipeline/slug.ts`, `REJECTED_DIR` from `src/pipeline/vaultDirs.ts`
  - `acquireLock` / `releaseLock` / `LOCK_PATH` from `src/pipeline/lock.ts`
- Produces:
  - `selectRejectsToApply(rows: DistilledRow[], audits: AuditRow[], project?: string): DistilledRow[]`
  - `interface ApplyRejectsSummary { moved: number; missingFile: number; collision: number }`
  - `applyAuditRejects(db: StateDb, vaultRoot: string, opts?: { project?: string; now?: string; lockPath?: string }): ApplyRejectsSummary`

- [ ] **Step 1: Write the failing tests.** Create `src/audit/apply.test.ts`:
...
Add to `src/cli/review.test.ts`, inside the existing `describe("rejections reach the database", ...)`:
...
Run: `npx vitest run src/audit/apply.test.ts src/cli/review.test.ts`
Expected: FAIL. `./apply.js` doesn't exist yet, and `rejected_by` survives the restore.

- [ ] **Step 3: Implement.** Create `src/audit/apply.ts`:
...
In `src/cli/review.ts`, `restoreRejected`: change `removeFrontmatterKeys(content, ["rejected_at"])` to `removeFrontmatterKeys(content, ["rejected_at", "rejected_by"])`.

In `src/cli.ts`, on the `audit` command:
1. Add `.option("--apply-rejects", "Move notes with a fresh reject verdict to .rejected/ (no model call; undo with vir review --restore)")`, and add `applyRejects?: boolean` to the options type.
2. At the top of the action, right after `vaultRoot` is resolved, add a branch that makes **no model call**:
...
```ts
        if (opts.applyRejects) {
          ...
          const s = applyAuditRejects(db, vaultRoot, { project: opts.project });
          ui.summary({
...
Note that the default is `false` here, where compose's confirm defaults to `true`, because this moves files. Import `applyAuditRejects` and `selectRejectsToApply` from `./audit/apply.js`. Use whatever local name Task 4 gave the vault root.

3. Docs:
   - README: add a row `| \`vir audit --apply-rejects\` | free | Move fresh reject verdicts to \`.rejected/\` (reversible) |`, and update the Tests row to the real count.
   - CHANGELOG `## Unreleased` entry: one bullet on `--apply-rejects`. It acts on rejects only, makes no model call, stamps `rejected_by: audit`, is undone by `vir review --restore`, and is gated on calibration rule 5.
   - Worktree `CLAUDE.md`: amend the `vir audit` convention bullet and the Commands block the same way.

- [ ] **Step 4: Run the tests and confirm they pass**

Run: `npx vitest run src/audit src/cli/review.test.ts && npx tsc --noEmit -p .`
Expected: PASS (8 new: 7 in apply.test.ts, 1 in review.test.ts). Then run the full suite and `npm run build`.

- [ ] **Step 5: Commit**

```bash
git add src/audit/apply.ts src/audit/apply.test.ts src/cli/review.ts src/cli/review.test.ts src/cli.ts README.md CHANGELOG.md
git commit -m "feat(audit): vir audit --apply-rejects moves fresh reject verdicts to .rejected/, reversibly"
```
...
- **Test count:** 4 + 10 + 7 + 2 = 23 new in Tasks 1–5 (807 → 830 at planning time), plus 1 in the Task 7 prompt fix and 8 in Task 8.
- **Task 8 vs design decision 1:** decision 1 is amended explicitly, and rule 5 gates Task 8's shipping.
